...
func (c *ClientConn) readPump() {
	...
	for {
		...
		go handleMessage(message, c.userID)
	}
}
...
func handleMessage(message []byte, userID string) {
	var msg Message
	...
	switch msg.Type {
	case MESSAGE, UPDATE_LAST_READ:
		if err := redis.Publish("chat:events", msg); err != nil {
